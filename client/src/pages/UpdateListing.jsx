...
export default function CreateListing() {
  ...
  const handleImageSubmit = (e) => {
    ...
    if(files.length > 0 && files.length < 7){
      ...
      Promise.all(promises)
        ...
        .catch((err) => {
          setImageUploadError('Image upload failed (max 2mb per per image)')
          setUploading(false)
        })
  ...
  useEffect(() => {
    ...
    const fetchDetails = async () => {
      const res = await fetch(`/api/v1/listing/get/${id}`)
      ...
      <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4'>
        <div className='flex flex-col gap-4 flex-1'>
          ...
          <div className='flex gap-6 flex-wrap'>
            <div className='flex gap-2'>
              <input type='checkbox' id='sell' className='w-5' checked={formData.type === 'sell'} onChange={handleChange}/>
              <span>Sell</span>
            </div>
            <div className='flex gap-2'>
        ...
        <div className="flex flex-col flex-1 gap-4">
          ...
          <div className="flex gap-4">
            ...
            <button type='button' disabled={uploading} onClick={handleImageSubmit} className='p-3 text-green-700 border border-green-700 rounded uppercase hover:shadow-lg disabled:opacity-80 hover:bg-green-700 hover:text-white duration-200'>{uploading ? 'uploading...' : 'upload'}</button>
          </div>
